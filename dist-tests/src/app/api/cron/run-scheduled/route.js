...
async function GET(req) {
    try {
        ...
        const activeWorkflows = await db_1.db.workflow.findMany({
            where: {
                status: "ACTIVE",
            },
            include: {
        ...
        const activeScheduledWorkflows = activeWorkflows.filter((wf) => {
        ...
        if (activeScheduledWorkflows.length === 0) {
            return server_1.NextResponse.json({ success: true, message: "No scheduled workflows to run" });
        }
        // 3. Execute workflows asynchronously
        const executions = activeScheduledWorkflows.map(async (wf) => {
            try {
                ...
                await workflow_execution_service_1.WorkflowExecutionService.executeWebhookTrigger(wf.organizationId, wf.id, { triggerSource: "cron" });
            }
            catch (error) {
