...
export function CosmicDustBackground({ className = "" }: { className?: string }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const { resolvedTheme } = useTheme();
  
  // Store uniforms in a ref so we can update them without recreating the whole scene
  const uniformsRef = useRef<any>(null);
  const finalUniformsRef = useRef<any>(null);
    ...
    (async () => {
      ...
      const FinalPass = {
        uniforms: {
          ...
          torusTexture: { value: null as Texture | null },
          bloomTexture: { value: null as Texture | null },
          haloTexture:  { value: null as Texture | null },
      ...
      function appearIn() {
        ...
        if (t < 1 && !disposed) requestAnimationFrame(appearIn);
      }
      appearIn();
